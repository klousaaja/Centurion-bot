...
namespace MainCore.UI.ViewModels.Tabs.Villages
{
    ...
    public partial class BuildViewModel : VillageTabViewModelBase
    {
        ...
        private async Task Import()
        {
            ...
            var confirm = await _dialogService.ConfirmBox.Handle(new MessageBoxData("Warning", "Centurion will remove resource field build job if its position doesn't match with current village."));
            if (!confirm) return;
